...
/**
 * 功能：校验单一真源是否有效，以及 3 份派生文档是否与当前渲染结果一致。
 * 输入：无。
...
function main() {
  ...
  const targets = [DOC_38_PATH, SESSION_HANDOFF_PATH, NEXT_STEPS_PATH];
  const mismatches = targets.filter((filePath) => fs.readFileSync(filePath, 'utf8') !== expected[filePath]);
